        ...
        <div className="flex gap-3 mb-3">
          ...
          <div className="flex flex-col items-center gap-1 shrink-0">
            <Avatar className="h-12 w-12">
              {lead.lineStatus === "joined" && lead.avatarUrl && (
                <AvatarImage src={lead.avatarUrl || "/placeholder.svg"} alt={lead.cxpName} />
              )}
              <AvatarFallback
                className={
                  lead.lineStatus === "joined" ? "bg-blue-100 text-blue-700 font-semibold" : "bg-gray-100 text-gray-400"
                }
              >
                {lead.lineStatus === "joined" ? getInitials(lead.cxpName) : <UserX className="h-5 w-5" />}
              </AvatarFallback>
            </Avatar>
            ...
            <div className="flex items-center gap-1">
              <FileCheck
                className={`h-4 w-4 ${
                  lead.testDriveConsent?.submittedAt ? "text-green-600 fill-green-600" : "text-gray-400"
                }`}
              />
              <span
                className={`text-xs ${
                  lead.testDriveConsent?.submittedAt ? "text-green-600 font-medium" : "text-gray-400"
                }`}
              >
                {lead.testDriveConsent?.submittedAt ? "已填" : "未填"}
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between mb-1">
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-foreground text-base mb-1">{lead.cxpName}</h3>
                <div className="flex items-center gap-1.5 text-sm">
                  <MessageCircle
                    className={`h-3.5 w-3.5 shrink-0 ${lead.lineStatus === "joined" ? "text-green-600" : "text-muted-foreground"}`}
                  />
                  <span className={lead.lineStatus === "joined" ? "text-foreground" : "text-muted-foreground"}>
                    {lead.lineStatus === "joined" ? lead.lineName : "未加入"}
                  </span>
                </div>
              </div>
              <Button
                size="sm"
                variant="outline"
                className="text-xs border-green-600 text-green-600 hover:bg-green-50 bg-transparent"
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm text-foreground">
                ...
                <span>886 {lead.phone.replace(/^0/, "").replace(/[-\s]/g, "")}</span>
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>建檔：{formatDate(lead.createdAt)}</span>
                </div>
      ...
      <Sheet open={isInviteSheetOpen} onOpenChange={setIsInviteSheetOpen}>
        <SheetContent side="bottom" className="rounded-t-xl px-4 h-[85vh] overflow-y-auto">
          <SheetHeader className="text-left pb-4">
            <SheetTitle>
              {lead.lineStatus === "joined" ? `${lead.cxpName} - 試駕邀請` : `邀請 ${lead.cxpName} 加入 LINE`}
            </SheetTitle>
          </SheetHeader>
          
          {lead.lineStatus === "not-joined" ? (
          ...
          /* Joined users - only show test drive invite content directly */
          <div className="space-y-4">
              {testDriveStep === "form" ? (
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="border-2 border-dashed rounded-lg p-4 text-center">
                      {licenseFrontPreview ? (
                        <div className="relative">
                          ...
                          <Button variant="ghost" size="sm" className="mt-2" onClick={() => setLicenseFrontPreview(null)}>移除</Button>
                        </div>
                      ) : (
                        <label className="cursor-pointer block">
                          <Upload className="h-8 w-8 mx-auto text-muted-foreground mb-2" />
                    ...
                    <div className="border-2 border-dashed rounded-lg p-4 text-center">
                      {licenseBackPreview ? (
                        <div className="relative">
                          ...
                          <Button variant="ghost" size="sm" className="mt-2" onClick={() => setLicenseBackPreview(null)}>移除</Button>
                        </div>
                      ) : (
                        <label className="cursor-pointer block">
                          <Upload className="h-8 w-8 mx-auto text-muted-foreground mb-2" />
